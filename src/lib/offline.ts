...
export function useOffline<T extends OfflineCapableData>(type: string) {
  ...
  useEffect(() => {
    ...
    const updateSyncStatus = () => setSyncStatus(offlineManager.getSyncStatus());
  ...
  const store = useCallback(async (data: T) => {
    return offlineManager.store(type, data);
  }, [type]);

  const get = useCallback(async (id: string) => {
    return offlineManager.get<T>(type, id);
  }, [type]);

  const getAll = useCallback(async () => {
    return offlineManager.getAll<T>(type);
  }, [type]);

  const remove = useCallback(async (id: string) => {
    return offlineManager.delete(type, id);
  }, [type]);

  const syncPending = useCallback(async () => {
    return offlineManager.syncPendingChanges();
  }, []);
